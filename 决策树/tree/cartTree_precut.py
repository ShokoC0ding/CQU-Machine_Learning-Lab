...
def current_accuracy(tree_node=TreeNode, test_data=[], test_label=[]):
    """
    计算当前决策树在训练数据集上的正确率
    ...
    for i in range(0, len(test_label)):
        this_label = cartTree.classify_data(root_node, test_data[i])
...
def finish_node(current_node, data=[], label=[], test_data=[], test_label=[]):
    """
    完成一个结点上的计算
    ...
    for title in rest_title:
        ...
        temp_data = data[0]
        this_gain, this_split_value = cartTree.gini_index(attr_values, current_label,
                                                          cartTree.is_number(temp_data[title]))  # 如果属性值为数字，则认为是连续的
    ...
    a_data = data[0]
    if cartTree.is_number(a_data[best_attr]):  # 如果是该属性的值为连续数值
    ...
    print('当前节点: '+current_node.attribute_name)
    print('剪枝前: '+str(before_accuracy))
    print('剪枝后: '+str(later_accuracy))
...
def run_test():
    ...
    print('训练的决策树是:')
    cartTree.print_tree(decision_tree)
    ...
    for melon in test_data:
        test_judge.append(cartTree.classify_data(decision_tree, melon))
