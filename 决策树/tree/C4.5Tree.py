...
def finish_node(current_node, data, label):
    ...
    for title in rest_title:
        ...
        title_IV[title] = ent(attr_values)
        title_gain[title] = this_gain
        title_split_value[title] = this_split_value

    gain_list  = title_gain.values()
    ave_gain = sum(gain_list)/len(gain_list)

    above_average_pairs = {k: v for k, v in title_gain.items() if v > ave_gain}

    gain_ratio_dict = {}
    for key in above_average_pairs:
        if key in title_IV:
            gain_ratio = above_average_pairs[key]/title_IV[key]
            gain_ratio_dict[key] = gain_ratio

    best_attr = max(gain_ratio_dict, key=gain_ratio_dict.get)  # 信息增益最大的属性名
